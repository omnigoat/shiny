	...
	: window_(window), width_(width), height_(height), fullscreen_()
{
	...
	enummerate_backbuffers();
	create_swapchain();

	on_resize_handle_ = window_->on_resize.connect([&](atma::event_flow_t&, uint32_t width, uint32_t height) {
...
{
	ATMA_ENSURE_IS(S_OK, detail::d3d_device_->QueryInterface(__uuidof(IDXGIDevice1), (void**)&dxgi_device_));
	ATMA_ENSURE_IS(S_OK, dxgi_device->GetParent(__uuidof(IDXGIAdapter1), (void**)&dxgi_adapter_));
	ATMA_ENSURE_IS(S_OK, dxgi_adapter->GetParent(__uuidof(IDXGIFactory1), (void**)&dxgi_factory_));

	auto format = closest_display_mode(width_, height_);
	...
	auto desc = DXGI_SWAP_CHAIN_DESC{
		...
		// DXGI_SAMPLE_DESC
		{1, 0},
		0, 3, window->hwnd, TRUE, DXGI_SWAP_EFFECT_DISCARD, 0
	};

	ATMA_ENSURE_IS(S_OK, dxgi_factory->CreateSwapChain(detail::d3d_device_, &desc, &dxgi_swap_chain_));
}
