...
namespace shiny_dx11
{
	...
	auto runtime_dx11_t::dxgi_factory() const -> platform::dxgi_factory_ptr const&
	{
		return dxgi_factory_;
	}

	auto runtime_dx11_t::enumerate_backbuffers(platform::dxgi_output_ptr const& output) -> void
	{
		UINT mode_count = 0;
	...
	auto shiny::runtime_dx11_t::dxgid3d_for_adapter(uint32 adapter_index) -> std::tuple<dxgi_adapter_ptr, d3d_device_ptr, d3d_renderer_ptr>
	{
	...
	auto runtime_dx11_t::dxgi_output_of(platform::dxgi_adapter_ptr const& adapter, uint output_index) -> platform::dxgi_output_ptr const&
	{
	...
	auto runtime_dx11_t::d3d_report_live_objects() -> void
	{
#if _DEBUG
		if (dxgi_debug)
	...
	auto runtime_dx11_t::make_data_declaration(shiny::data_streams_t const& streams) -> data_declaration_t const*
	{
	...
	auto runtime_dx11_t::geometry_declaration_of(shiny::geometry_streams_t const& streams) -> geometry_declaration_t const*
	{
