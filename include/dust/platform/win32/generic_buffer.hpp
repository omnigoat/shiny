...
namespace dust
{
	struct generic_buffer_t : dust::buffer_t
	{
		...
		generic_buffer_t(context_ptr const& ctx, buffer_usage_t usage, uint stride, uint elements, void const* data, uint data_elemcount);
	...
	inline auto create_generic_buffer(
		context_ptr const& ctx, buffer_usage_t usage, uint stride,
		uint elements, void const* data, uint data_elemcount)
		-> generic_buffer_ptr
	{
